...
{ 
  ...
  i2c_master_init();
  uint8_t res[10];
  ma_read_byte(0x20,1,MA_hw_version__a,res);
  printf("Hardware version: 0x%02x\n",res[0]);

  ma_write_byte(0x20,1,MA_i2s_format__a,8);          // Set i2s left justified, set audio_proc_enable
  ma_write_byte(0x20,1,MA_vol_db_master__a,0x20);    // Set vol_db_master 

  ma_read_byte(0x20,1,MA_error__a,res); 
  ...
  printf("Done init ma120x0____________________________________________________\n");
}
...
esp_err_t ma_read_byte(uint8_t i2c_addr, uint8_t prot, uint16_t address, uint8_t *rbuf) {
  uint8_t value = 0;
  esp_err_t ret;
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
