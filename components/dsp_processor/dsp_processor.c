...
static void dsp_i2s_task_handler(void *arg) {
  ...
  ESP_LOGI(TAG,"Will enter audio backend loop");
  for (;;) {
    ...
    if (timestampSize == NULL) {
      ESP_LOGI("Backend", "Wait: no data in buffer %d %d", cnt, n_byte_read);
      continue;
    }

    if (n_byte_read != 12) {
      ESP_LOGE("backend", "error read from ringbuf %d ", n_byte_read);
    ...
    //ESP_LOGI(TAG,"Got time stamp from server - TS: %d.%d - size:%d",ts_sec,ts_usec, ts_size);  
    // what's this for?
    //while (tdif.tv_sec == 0) {
    //  vTaskDelay(1);
    //}
    ...
    if (age < 0 ) { age = 0; } // Fix first wrong time stamp value as invalid 
    ...
    if (chunk_size != ts_size) {
      ...
      uint8_t *ax = (audio + chunk_size) ; 
      ax = (uint8_t *)xRingbufferReceiveUpTo(s_ringbuf_i2s, &chunk_size,(portTickType)20, missing);
...
// FLAC 
//#define BUFFER_SIZE 192 * (4608 + 12)    // 44100/16/2
...
void dsp_i2s_task_init(uint32_t sample_rate, bool slave) {
  ...
  printf("Start audio backend\n");
  xTaskCreatePinnedToCore(dsp_i2s_task_handler, "DSP_I2S", 40 * 1024, NULL, 5,
                          &s_dsp_i2s_task_handle,0);
  // xTaskCreate(dsp_i2s_task_handler, "DSP_I2S", 48*1024, NULL, 7,
  // &s_dsp_i2s_task_handle);
  printf("Audio backend started\n");
}
