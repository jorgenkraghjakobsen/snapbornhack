...
static char FlacPCMBuffer[6000]; 
...
codec_header_message_t* flacHeader = NULL;
wire_chunk_message_t* flacChunk = NULL;
wire_chunk_message_t* pcmChunk = NULL;
//SampleFormat sampleFormat;

// callbacks
static void flac_metadata_cb(const FLAC__StreamDecoder* decoder, const FLAC__StreamMetadata* metadata, void* client_data) 
{ 
  ESP_LOGI(TAG, "flac metadata cb");  
  if (metadata->type == FLAC__METADATA_TYPE_STREAMINFO)
  {  ESP_LOGI(TAG, "Samplerate         : %d ", metadata->data.stream_info.sample_rate); 
     ESP_LOGI(TAG, "Bits pr sample     : %d ", metadata->data.stream_info.bits_per_sample); 
     ESP_LOGI(TAG, "Number of channels : %d ", metadata->data.stream_info.channels); 
  }
}
...
{ if (flacHeader != NULL) {
    ESP_LOGI(TAG,"FLAC Read_callback Header");
    *bytes = flacHeader->size;
    memcpy(buffer, flacHeader->payload, *bytes);
    flacHeader = NULL;
  }  
  else if (flacChunk != NULL) { 
    //ESP_LOGI(TAG,"FLAC Read_callback Chunk bytes in   :%d",*bytes);
    if (*bytes > flacChunk->size) 
        *bytes = flacChunk->size; 
    /* char* dbuf = flacChunk->payload; 
    for (int i = 0; i<16 ; i++)
    { printf("%02x ", *(dbuf++));
    } 
    printf("\n"); */ 
    memcpy(buffer, flacChunk->payload, *bytes);
    memmove(flacChunk->payload, flacChunk->payload + *bytes, flacChunk->size - *bytes);
    flacChunk->size = flacChunk->size - (uint32_t)(*bytes);
    flacChunk->payload = (char*)realloc(flacChunk->payload, flacChunk->size);
  }
...
{  //ESP_LOGI(TAG,"FLAC Write_callback ");
   
   
   //size_t bytes = frame->header.blocksize * 4 ; // sampleFormat.frameSize();
   //Use FlacPCMbuffer  instead of pcmChunk->payload = (char*)realloc(pcmChunk->payload, pcmChunk->size + bytes);
  ...
  for (size_t i = 0; i < frame->header.blocksize; i++) {
    ...
    FlacPCMBuffer[i*4+0] = (char)(ui16);
    FlacPCMBuffer[i*4+1] = (char)(ui16 >> 8);

    i16 = (FLAC__int16)buffer[1][i];
    ui16 = (FLAC__uint16)i16;

    FlacPCMBuffer[i*4+2] = (char)(ui16);
    FlacPCMBuffer[i*4+3] = (char)(ui16 >> 8);
   }
  
   uint32_t pcmSize = (uint32_t) (frame->header.blocksize * 4); 
   uint8_t timestampSize[12];
   memcpy(timestampSize + 0, &flacChunk->timestamp.sec,
                       sizeof(flacChunk->timestamp.sec));
   memcpy(timestampSize + 4, &flacChunk->timestamp.usec,
                       sizeof(flacChunk->timestamp.usec));
   memcpy(timestampSize + 8, &pcmSize, sizeof(uint32_t));
   //ESP_LOGI(TAG, "Time stamp in from socket: %d",flacChunk->timestamp.sec);
   uint32_t chunk_res; 
   ...
   if ((chunk_res = write_ringbuf((const uint8_t *) FlacPCMBuffer,
                                               pcmSize)) != pcmSize) {
        ESP_LOGI(TAG, "Error writing data to ring buffer: %d", chunk_res);
   }
   //ESP_LOGI(TAG,"Write_callback data written ");
...
static void flac_error_cb(const FLAC__StreamDecoder* decoder, FLAC__StreamDecoderErrorStatus status, void* client_data) {
      ESP_LOGE(TAG, " Got error callback: %s ", FLAC__StreamDecoderErrorStatusString[status]); 
}
...
static void http_get_task(void *pvParameters) {
  ...
  FLAC_Data* client_data;
  client_data = (FLAC_Data *)malloc (sizeof (FLAC_Data));
  ...
  while (1) {
    ...
    */
    ...
    inet_pton(AF_INET, "151.216.14.103", &(servaddr.sin_addr.s_addr));
        ...
        case SNAPCAST_MESSAGE_CODEC_HEADER:
          ...
          if (strcmp(codec_header_message.codec, "opus") == 0) {
            ...
            codec = PCM;
             ...
             memcpy(&bits, start + 8, sizeof(bits));
             memcpy(&channels, start + 10, sizeof(channels));
             ESP_LOGI(TAG, "Raw PCM audio stream: %d:%d:%d\n", rate, bits,
                     channels);
          
          } else if (strcmp(codec_header_message.codec, "flac") == 0) {
            
            flacHeader = &codec_header_message; 

            ESP_LOGI(TAG, "Codec is FLAC");
            if ((decoder = FLAC__stream_decoder_new()) == NULL) 
            { 
              ESP_LOGE(TAG, "Can not create flac decoder instance");
            }
            FLAC__StreamDecoderInitStatus init_status;
            if ( (init_status = FLAC__stream_decoder_init_stream(decoder, flac_read_cb, NULL, NULL, NULL, NULL, flac_write_cb,
                                                   flac_metadata_cb, flac_error_cb, client_data
               )) != FLAC__STREAM_DECODER_INIT_STATUS_OK) { 
                  ESP_LOGE(TAG, "ERROR: initializing decoder: %s ", FLAC__StreamDecoderInitStatusString[init_status]); 
            }  
            ...
            ESP_LOGI(TAG,"FLAC header message done");
            ...
            ESP_LOGI(TAG,
                     "Change encoder codec to opus flac or pcm in /etc/snapserver.conf on "
                     "server\n");
            return;
          }
        ...
        case SNAPCAST_MESSAGE_WIRE_CHUNK:
          ...
          switch (codec) {
            case OPUS:
              ...
              break;
            case FLAC: 
              //ESP_LOGI(TAG,"FLAC chunk messeage in - Size:%d",wire_chunk_message.size); 
              //pcmChunk = &wire_chunk_message;
              flacChunk = &wire_chunk_message;
              //flacChunk->payload = (char*)malloc(size);
              //flacChunk->payload = (char*)realloc(flacChunk->payload, size);
              memcpy(flacChunk->payload, start, size);
              flacChunk->size = size;
              
              //pcmChunk->payload = (char*)realloc(pcmChunk->payload, 0);
              //pcmChunk->size = 0;
              
              //ESP_LOGI(TAG,"size, %d",flacChunk->size);

              while (flacChunk->size > 0)
              {   //ESP_LOGI(TAG,"In while %d",flacChunk->size);
                  if (!FLAC__stream_decoder_process_single(decoder))
                  {
                      ESP_LOGI("STREAMER_FLAC","Decoder process single ERROR"); 
                  
                      return false;
                  }
                  //ESP_LOGI("STREAMER_FLAC","Decoder process single done"); 
                  /*if (lastError_)
                  {
...
void app_main(void) {
  ...
  uint32_t backend_state = 0;
  xQueueSend(flow_queue, &backend_state, 10);
  //xTaskCreate(&ota_server_task, "ota_server_task", 4096, NULL, 15, NULL);
  
  xTaskCreatePinnedToCore(&http_get_task, "http_get_task", 3 * 4096, NULL, 4,
                          &t_http_get_task, 1);
  while (1) {
